...
function HourlyWeather() {
  ...
  const [hours, setHours] = useState(null); // Final times to be rendered in UI
  const [finalTemps, setFinalTemps] = useState(null); // Final temps to be rendered in UI
  const [finalWeather, setFinalWeather] = useState(null); // Final weather conditions to be rendered in UI
  ...
  function getConditionIcon(weatherCode, time) {
    ...
    switch (weatherCode) {
      ...
      case 1:
        return isDay ? (
          <ClearDayStatic className={classes.hourlyIcon} />
        ) : (
      ...
      case 2:
        return isDay ? (
          <PartlyCloudyDayStatic className={classes.hourlyIcon} />
        ) : (
          <PartlyCloudyNightStatic className={classes.hourlyIcon} />
        );
        break;
      case 3:
        return isDay ? (
          <OvercastDayStatic className={classes.hourlyIcon} />
        ) : (
          <OvercastNightStatic className={classes.hourlyIcon} />
        );
        break;
      case 45:
      case 48:
        return isDay ? (
          <FogDayStatic className={classes.hourlyIcon} />
        ) : (
          <FogNightStatic className={classes.hourlyIcon} />
        );
        break;
      case 51:
      case 53:
      ...
      case 99:
        return isDay ? (
          <ThunderstormsDayStatic className={classes.hourlyIcon} />
        ) : (
          <ThunderstormsNightStatic className={classes.hourlyIcon} />
        );
        break;
      case 100:
  ...
  useEffect(() => {
    ...
    fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=temperature_2m,weather_code&temperature_unit=${unit}&wind_speed_unit=mph&precipitation_unit=inch&past_days=1&timezone=auto&forecast_days=3&daily=sunrise,sunset`
    )
      .then((response) => {
        if (!response.ok) {
    ...
    if (dailyData.sunrise && dailyData.sunset) {
      for (let i = 0; i < dailyData.sunrise.length; i++) {
        ...
        if (dayIter.getDay() === currentDay) {
          ...
          const nextSunrise =
            currentDate < currentSunrise
              ? dailyData.sunrise[i]
              : dailyData.sunrise[i + 1];
          const nextSunset =
            currentDate < currentSunset
              ? dailyData.sunset[i]
              : dailyData.sunset[i + 1];
          ...
          <ul className={classes.hourlyForecast}>
            {hours &&
              hours.map((hour, index, hours) => {
                ...
                const pastHour = new Date(hours[index - 1]); // what if index is 0?
                ...
                if (currentHour.getHours() === pastHour.getHours()) {
                  ...
                  return (
                    <li key={index}>
                      ...
                      <div>
                        {finalWeather[index] === 100 ? "Sunrise" : "Sunset"}
                      </div>
                    </li>
                  );
                } else {
